...
def LoadHatenadirStructure(Resource, path=os.path.join("hatenadir", "ds", "v2-xx")):
    # Clear the existing resource children
    Resource.children = {}
    ...
    for root, dirs, files in os.walk(path):
        ...
        for filename in files:
            ...
            if filetype == "ugoxml":
                Resource.putChild(filename[:-3], UGOXMLResource(filepath))
            elif filetype == "py":
                try:
                    pyfile = imp.load_source("pyfile", os.path.abspath(filepath))
                ...
                Resource.putChild(filename, FileResource(filepath))
