...
class CreatorIDFileResource(resource.Resource):
	...
	def render(self, request):
		...
		if filetype in "ppm":
			...
			#comment_data = Database.GetCommentPPM(creator, file[:-4])  # read comment ppm data
			#send file to the client:
			request.responseHeaders.setRawHeaders('content-type', ['text/plain'])
			return data #+ comment_data  # return both ppm and comment ppm data
		elif filetype == "info":
			path = "/".join(request.path.split("/")[3:])
			...
			#undocumented html responses? mt are you fucking stupid??? why didnt you comment this when you knew what it did???????
			if "mode" in request.args:
				if request.args["mode"][0] == "commentshalfsize":
					#loads the htm generated profile:
					php_file_path = 'dsphp\commentsds.php'
					args = {
						'creatorid': '%s' % (self.CreatorID),  # creatorid
						'comments': '%s' % (self.Comments),#comment number (not working for now)
						'flipnote': '%s' % (self.filename) #filename
						}	
					html = execute_php_file(php_file_path, args)
					if html:
						print("%s send php file whit following args %s" % (request.getClientIP(), args))
						return html.encode("UTF-8")
					ServerLog.write("requested comments?")
				if request.args["mode"][0] == "profiles":
					#generates the profile notes:
					php_file_path = 'hatenadir\ds\\v2-xx\\frontpage\profiles\profile.php'
					args = {
					...
					if html:
						print("%s send php file whit following args %s" % (request.getClientIP(), args))
						print(html.encode("UTF-16LE"))
					
					ServerLog.write("generated the profile.ugo waos")
					LoadHatenadirStructure(self)
					#loads the htm generated profile:
					php_file_path = 'dsphp\userds.php'
					args = {
						'username': '%s' % (self.tmb.Username)  # Pass tmb.Username as an argument
						}	
					html = execute_php_file(php_file_path, args)
	...
	def GenerateDetailsPage(self, CreatorID, filename):#filename without ext
		...
		self.tmb = TMB().Read(Database.GetFlipnoteTMB(CreatorID, filename))
		if not tmb:
		...
		content = '<a href="http://flipnote.hatena.com/ds/v2-eu/movie/%s/%s.htm?mode=profiles">%s</a>' % (CreatorID, filename, tmb.Username)
