...
class Database:
	...
	def GetFlipnotePPM(self, CreatorID, filename):#the ppm binary data
		...
		f.close()
		return ret
	def GetCommentPPM(self, creator_id, flipnote_id, filename):#the ppm binary data
		f = open(self.CommentPath(creator_id, flipnote_id, filename), "rb")
		ret = f.read()
		f.close()
		return ret
	...
	def GetFlipnoteTMB(self, CreatorID, filename):#the tmb binary data
		...
		f.close()
		return ret
	def GetCommentTMB(self, CreatorID, filename):#the tmb binary data
		f = open(self.CommentPath(CreatorID, filename), "rb")
		ret = f.read(0x6a0)
		f.close()
		return ret	
	def AddFlipnote(self, content, Channel=""):#content = ppm binary data
		...
		f.write(content)
		f.close()
		# write username to authors.dat
		with open("database/authors.dat", "a") as f:
			f.write("%s\t%s\n" % (filename, username))
		return CreatorID, filename
	def AddComment(self, content, creator_id,  flipnote_id, Channel=""):#content = ppm binary data
		tmb = TMB().Read(content)
		if not tmb:
			return False
		print(self)
		...
		comment_dir = "hatenadir/ds/v2-xx/comments/" + creator_id + "/" + flipnote_id + "/comments"
		try:
		    os.makedirs(comment_dir)
		except OSError:
			pass
		#write comment to file:
		f = open(os.path.join(comment_dir, filename + ".ppm"), "wb")
		f.write(content)
		f.close()
		#the writing to comments.dat is useles sience the comments.php just reads the folder but some one might find it usefull i guess
		# write comment to comments.dat
		with open("database/comments.dat", "a") as f:
			#the first filename is supposed to be the flipnote it belongs to, but i did not figure out how to pass that value
			#if you figure out how to get this working this way ill give you an imaginary cookie!!!!  -mtboss
			#f.write("%s\t%s\t%s\t%s\n" % (filename, username, filename, datetime.date.today().strftime("%m/%d/%Y")))
			f.write("%s\t%s\t%s\n" % (username, filename, datetime.date.today().strftime("%m/%d/%Y")))
		return CreatorID, filename
	...
	def CommentPath(self, creator_id, flipnote_id, filename):#use self.GetCommentPPM() instead
		return "hatenadir/ds/v2-xx/comments/%s/%s/comments/%s.ppm" % (creator_id, flipnote_id, filename)
